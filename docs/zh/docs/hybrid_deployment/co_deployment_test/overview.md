...
为提升计算资源利用率，openEuler 支持在线任务与离线任务的混合部署（简称“混部”）。然而，混部面临的一个关键挑战是如何量化离线任务对在线任务的性能干扰，这对于确保该特性在生产环境中安全、可控地上线至关重要。
...
2. 限制离线网络带宽
    ...
    ```bash
    bwmcli -s bandwidth 10MB,1GB //有在线任务10MB/s，无在线任务是1GB/s（根据实际网络带宽修改）
    ```

3. 配置当在线达到20MB/s时，开始限制离线带宽10MB/s以内;若低于20MB/s，则限制离线1GB/s以内
...
结论：混部CPU隔离后，在线任务QPS干扰率从14.18%下降到4.73%，平均时延干扰从17.26%下降到4.42%，P99干扰率从28.33%下降到1.65%。
...
* 结论：混部IO隔离后，在线任务QOS干扰率从77.12%下降到23.19%，平均时延干扰率从337.49%下降到30.19%，P95时延干扰率从307.36%下降到33.39%。
